...
class KerasRNN(object):
    ...
    def main(self, embed_hidden_size, rnn_size, optimizer_name="adam", learning_rate=0.0001,
             ...
             prediction_filename="scorer/keras_rnn.pred"):
        if test:
            X_train_Q, X_train_A, y_train, X_test_Q, X_test_A, _, test_idx, vocab_size = self.data_loader.get_data_separate_sentences_test()
        else:
            X_train_Q, X_train_A, y_train, X_test_Q, X_test_A, _, test_idx, vocab_size = self.data_loader.get_data_separate_sentences()
        print(y_train)
